...
void onTick(CBlob@ this)
{
	...
	if(getNet().isServer())
	if(this.getPlayer() is null)this.getBrain().server_SetActive(true);
	...
	if(this.isKeyPressed(key_action1)){
		...
		this.getSprite().SetEmitSound("fixing.ogg");
		this.getSprite().SetEmitSoundPaused(false);
	} else {
		this.Untag("repairing");
		this.getSprite().SetEmitSoundPaused(true);
		
		if(getNet().isServer())if(getGameTime() % 60 == 0){
			CBlob@[] blobsInRadius;
			if (this.getMap().getBlobsInRadius(this.getPosition(), 32.0f, @blobsInRadius)) 
			{
				for (uint i = 0; i < blobsInRadius.length; i++)
				{
					...
					if(b !is null){
						if(b.getTeamNum() != this.getTeamNum() && b.hasTag("flesh"))
						if(!getMap().rayCastSolid(b.getPosition(), this.getPosition())){
							
							string name = "crewlaser";
							
							if(this.getName() == "engi")name = "engilaser";
							if(this.getName() == "mantis")name = "mantisspit";
							
							CBlob @laser = server_CreateBlob(name,this.getTeamNum(),this.getPosition());
							Vec2f dir = b.getPosition()-this.getPosition();
							dir.Normalize();
							laser.setVelocity(dir*1.0f);
							break;
						}
					}
				}
