...
void onTick(CSprite@ this)
{
	...
	if (blob.hasTag("dead"))
	{
		...
		if (heatsig !is null)heatsig.SetVisible(false);
		return;
	}
	...
	if (heatsig !is null)
	{
		...
		if(getLocalPlayer().getBlob() !is blob){
			...
			if(local.getName() == "slug"){
				if(blob.getDistanceTo(local) > 60 || getMap().rayCastSolid(blob.getPosition(), local.getPosition())){
					heatsig.SetVisible(true);
					heatsig.setRenderStyle(RenderStyle::additive);
					heatsig.SetLighting(false);
					//blob.SetLight(true);
