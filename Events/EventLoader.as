...
void onInit(CBlob @ this){
	...
	this.set_string("filename","event"+XORRandom(14)+".png");
	this.set_u8("team",1);
...
void onTick(CBlob @ this){
	...
	if(this.get_u16("timer") >= 10*30 && this.get_u8("Y") < getMap().tilemapheight){
		wipeWorld(getMap(),this.get_u8("Y"),this.get_u8("Y")+1);
		
		this.set_u8("Y",this.get_u8("Y")+1);
	}
	
	
	if(!this.hasTag("stop"))
	if(this.get_u16("timer") >= 20*30 && this.get_u8("Y") >= getMap().tilemapheight){
		SetScreenFlash(255, 255, 255, 255);
